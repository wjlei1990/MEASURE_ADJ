...
!> measure_adj
!!measure_adj subroutine based on asdf data format
!!Input:
!> @param[in] data  observed seismogram
!! @param[in] npt1  npoints of obsd data
!! @param[in] t01   begin time of obsd data
!! @param[in] dt1   time_step of obsd data
!! @param[in] syn   synthetic seismogram
!! @param[in] npt2  npoints
!! @param[in] t02   begin time
!! @param[in] dt2   time step
!! @param[in] dist  distance from station to event
!! @param[in] sta   station name(sta, net, chan_dat are requested by original
!version of meaure_adj to write out extra information files)
!! @param[in] net   network name
!! @param[in] chan_dat      observed channel name(LHZ, LHR, LHT)
!! @param[in] win_all       win info of one pair of data generated by flexwin
!! @param[in] ma_par_all    measure_adj parameter struct
!! @param[in] weighting_par       weighting variables
!! @param[in] weighting_option    weighting option
!! @param[in] ma_outdir     measure_adj output dir(requested by original version)
!! output:
!! @param[out] win_chi_all   win_chi_all struct, contains all the win_chi info
!! @param[out] adj_source   adj_seismogram trace
...
subroutine read_ma_parfile(measure_adj_par_all,min_p,max_p,event_dpt)
  ...
  !for test purpose
  !fn=trim(parfile_dir)//"/MEASUREMENT.PAR"
  !print *, "fn:", trim(fn)
...
subroutine copy_weighting_par_to_local(weighting_par)
  ...
  num_Love_T = weighting_par%num_Love_T
